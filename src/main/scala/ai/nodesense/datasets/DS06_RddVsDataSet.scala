...
object DS06_RddVsDataSet {
  ...
  def main(args: Array[String]) {
    ...
    val rdd = sparkContext.textFile(FileUtils.getInputPath("text.txt"))

    val ds = sparkSession.read
                        .text(FileUtils.getInputPath("text.txt")).as[String]
    ...
    println("RDD Word List ", wordCount.collect.toList)
    ...
    val filteredRDD = wordsRDD.filter(value => value =="scala")
    println(filteredRDD.collect().toList)

    val filteredDS = wordsDs.filter(value => value =="scala")
    filteredDS.show()
